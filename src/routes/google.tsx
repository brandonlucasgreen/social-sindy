...
function notConfigured(c: AppContext) {
  return c.html(
    <Layout title="Google push unavailable — buffer-cally" user={c.get('user')} narrow>
...
function pushError(c: AppContext, error: unknown) {
  ...
  return c.html(
    <Layout title="Google sync failed — buffer-cally" user={c.get('user')} narrow>
...
googleRoutes.get('/google/callback', async (c) => {
  ...
  if (denied) {
    return c.html(
      <Layout title="Google not connected — buffer-cally" user={user} narrow>
